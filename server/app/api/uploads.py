...
def download_evidence_bundle(upload_id: str) -> StreamingResponse:
    ...
    with ZipFile(buffer, mode="w", compression=ZIP_DEFLATED) as archive:
        ...
        _write_evidence_file(archive, record, "invoice_arabic_bilingual_visual.pdf")
        archive.writestr(
            "evidence.json",
...
def generate_output(upload_id: str) -> EvidenceBundlePreview:
    ...
    if record.selected_country_pack == "saudi_zatca":
        ...
        _mark_generated_file(record, "invoice_arabic_bilingual_visual.pdf", relative_storage_path(pdf_path), pdf_hash)
...
def read_generated_pdf(upload_id: str) -> FileResponse:
    return _generated_evidence_file(upload_id, "invoice_arabic_bilingual_visual.pdf", "application/pdf", "Saudi visual PDF")
